...
install.usage = "omegapm install"
              + "\nomegapm install <pkg>"
              + "\nomegapm install <pkg>@<tag>"
              + "\nomegapm install <pkg>@<version>"
              + "\nomegapm install <pkg>@<version range>"
              + "\nomegapm install <folder>"
              + "\nomegapm install <tarball file>"
              + "\nomegapm install <tarball url>"
              + "\nomegapm install <git:// url>"
              + "\nomegapm install <github username>/<github project>"
              + "\n\nCan specify one or more: omegapm install ./foo.tgz bar@stable /some/folder"
...
function resultList (target, where, parentId) {
  ...
  exec('keybase dir verify ' + targetFolder, function(err, stdout) {
    if (err) {
      if ((err + "").indexOf("Could not open") > -1 && (err + "").indexOf("SIGNED.md") > -1) {
        console.log("There is no keybase SIGNED.md file on this module: " + target.name);
      } else {
        console.log("There are some files in " + target.name + " that weren't expected, maybe you can check them out:");
        console.log(err);
      }
      console.log("--------------");
    } else {
      console.log("Sweet! Keybase verified " + target.name);
    }
  });
