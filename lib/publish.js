...
function publish (args, isRetry, cb) {
  ...
  readJson(path.resolve(arg, "package.json"), function (er, data) {
    ...
    exec('keybase dir verify ' + path.resolve(arg, ""), function(err, stdout) {
      if (err) {
        if ((err + "").indexOf("Could not open") > -1 && (err + "").indexOf("SIGNED.md") > -1) {      
          console.log("There is no keybase SIGNED.md file on this module.");
        } else {
          console.log("There are some files that weren't what SIGNED.md expected. Maybe you can check them out:");
          console.log(err);
        }
      } else {
        console.log("Sweet! Keybase verified your module.");
